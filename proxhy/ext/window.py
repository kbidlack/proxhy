...
from ..proxy import listen_client


class WindowHandler(Proxhy):
...
class Window:
    ...
    def open(self):
        ...
        while self.window_id in self.proxhy.windows:
        ...
        self.proxhy.windows.update({self.window_id: self})
        self._open = True
...
class SettingsMenu(Window):
    def __init__(
        ...
        if num_slots % 9 != 0:
            raise ValueError(f'Expected multiple of 9 for num_slots; got {num_slots} instead.')
        super().__init__(proxhy, "Settings", "minecraft:chest", num_slots)
        self.num_slots = num_slots
        self.proxhy = proxhy
        self.settings = proxhy.settings
    ...
    def build(self):
        self.settings = self.proxhy.settings  # re-initialize settings so this can rebuild when settings update
    ...
    @staticmethod
    def get_setting_toggle_msg(s_display, old_state, new_state, old_state_color, new_state_color) -> TextComponent:
        toggle_msg = (
    ...
    def get_formatted_items(self) -> list[dict]:
        ...
        n_settings = sum([1 for s in self.subsettings.values() if (isinstance(s, dict) and "states" in s)])
        n_groups = sum([1 for s in self.subsettings.values() if (isinstance(s, dict) and not "states" in s)])
        ...
        if slots_needed > self.num_slots:
            ...
            raise OverflowError(f"Got {n_settings} settings and {n_groups} groups; can't fit into {self.num_slots} slots! ({slots_needed} slots required)")
        if n_settings % 2 == 0:
            is_even = True
        else:
            is_even = False
        ...
        for i, s in enumerate(subsettings_non_metadata):
            ...
            display_nbt = {  # display item
                "display": {
                    "Name": f"§r§l{s['display_name']}",
                    "Lore": lore
                }
            }
            ...
            items.append({
                "slot": slot + 9,
                "slot_data": SlotData(
                    Item.from_name(s["item"]),
                    nbt=dumps(from_dict(display_nbt))
                ),
                "callback": self.toggle_state_callback
            })

            items.append({  # state display glass pane, above display item
                "slot": slot,
    ...
    def toggle_state_callback(
        ...
        try:
            ...
            raise KeyError(f'Slot {slot+1} has no associated setting.\nSettings: {self.setting_slots}')
        ...
        msg = self.get_setting_toggle_msg(s_display, prev_state, next_state, prev_color, next_color)

        self.proxhy.client.chat(msg)
