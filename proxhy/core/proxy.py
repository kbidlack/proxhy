...
class PacketListenerRegistry:
    """Instance-specific packet listener registry."""
    
    def __init__(self):
        self.client_listeners: Dict[Tuple[int, State], Set[Tuple[Callable, bool]]] = defaultdict(set)
        self.server_listeners: Dict[Tuple[int, State], Set[Tuple[Callable, bool]]] = defaultdict(set)
    
    def register_client_listener(
        self, packet_id: int, state: State, func: Callable, blocking: bool = False, override: bool = False
    ):
        """Register a client packet listener."""
        if override:
            self.client_listeners[(packet_id, state)] = set()
        self.client_listeners[(packet_id, state)].add((func, blocking))
    
    def register_server_listener(
        self, packet_id: int, state: State, func: Callable, blocking: bool = False, override: bool = False
    ):
        """Register a server packet listener."""
        if override:
            self.server_listeners[(packet_id, state)] = set()
        self.server_listeners[(packet_id, state)].add((func, blocking))
    
    def get_client_listeners(self, packet_id: int, state: State) -> Set[Tuple[Callable, bool]]:
        """Get client packet listeners."""
        return self.client_listeners.get((packet_id, state), set())
    
    def get_server_listeners(self, packet_id: int, state: State) -> Set[Tuple[Callable, bool]]:
        """Get server packet listeners."""
        return self.server_listeners.get((packet_id, state), set())


def listen_client(
    packet_id: int, state: State = State.PLAY, blocking=False, override=False
):
    """Decorator for client packet listeners (for backward compatibility)."""
    def wrapper(func):
        # This will be registered when the proxy is created
        func._packet_listener_info = ('client', packet_id, state, blocking, override)
        return func
    return wrapper
...
def listen_server(
    ...
    """Decorator for server packet listeners (for backward compatibility)."""
    def wrapper(func):
        # This will be registered when the proxy is created
        func._packet_listener_info = ('server', packet_id, state, blocking, override)
        return func
    return wrapper
...
class Proxy:
    ...
    def __init__(
        ...
        # Instance-specific listener registry
        self.listeners = PacketListenerRegistry()
    ...
    async def handle_client(self):
        while packet_length := await VarInt.unpack_stream(self.client):
            if data := await self.client.read(packet_length):
                ...
                # call packet handler
                listeners = self.listeners.get_client_listeners(packet_id, self.state)
                if listeners:
                    for handler, blocking in listeners:
                        if blocking:
    ...
    async def handle_server(self):
        ...
        while packet_length := await VarInt.unpack_stream(self.server):
            ...
            # call packet handler
            listeners = self.listeners.get_server_listeners(packet_id, self.state)
            if listeners:
                for handler, blocking in listeners:
                    if blocking:
                        await handler(self, Buffer(packet_data))
