...
3. Commit (and push) the merge bump change to the dev branch (can be committed with other changes).
4. `git checkout main` and `git merge dev -m "release <VERSION>" --no-ff`.
5. `git tag v<VERSION>`.
6. `git push origin main` and `git push origin main --tags`.
7. Create a new release on GitHub with the new tag and title it with the release number (no v).
8. `git checkout dev`, `git merge main`, `git push origin dev`.
